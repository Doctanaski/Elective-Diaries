...
export default function HospitalCarousel({ hospitals }: { hospitals: Hospital[] }) {
  ...
  return (
    <div className="w-full">
      ...
      {/* ══════════════════════════════════════════
          MOBILE (< md) — CSS only, zero Motion
          ══════════════════════════════════════════ */}
      <div className="md:hidden flex flex-col rounded-2xl overflow-hidden border border-white/5">
        ...
        >
          {/* Stack all images, CSS opacity transition on active */}
            ...
            <div className="absolute top-3 right-3 z-10 flex gap-1.5">
              {hospitals.map((_, i) => (
                <button key={i} type="button" onClick={() => goTo(i)}
                  ...
                  style={{ width: i === active ? 20 : 6, backgroundColor: i === active ? '#fff' : 'rgba(255,255,255,0.4)' }}
                />
              ))}
            </div>
          )}
          ...
          {count > 1 && (
            <>
              <button type="button" onClick={goPrev}
                className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full
                           flex items-center justify-center bg-black/40 text-white border border-white/20">
                <span className="material-symbols-outlined" style={{ fontSize: 18 }}>chevron_left</span>
              </button>
              <button type="button" onClick={goNext}
                className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full
                           flex items-center justify-center bg-black/40 text-white border border-white/20">
        ...
        {/* Info panel — CSS transition on content swap */}
        <div className="bg-surface-container-low px-5 py-5">
          {/* Use CSS opacity on individual panels — no JS animation overhead */}
          {hospitals.map((hospital, i) => {
            const s = statusConfig[hospital.status] ?? statusConfig.inactive
            return (
              <div key={hospital.id}
              ...
              >
                {hospital.status !== 'inactive' && (
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-label text-[10px] uppercase tracking-widest text-white/50">{s.label}</span>
                  </div>
                )}
                ...
                <Link href={`/hospitals/${hospital.slug}`} prefetch
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl
                             bg-white text-black font-label text-sm font-bold active:scale-95 transition-transform">
                  View Diaries
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                ...
                <h2 className="font-headline font-extrabold text-4xl lg:text-5xl text-white
                               leading-tight tracking-tight mb-4">{h.name}</h2>
              ...
              {hospitals.map((hospital, i) => (
                <button key={hospital.id} type="button" onClick={() => goTo(i)}
                  className={`text-right font-label text-sm transition-all duration-300 px-3 py-1.5 rounded-lg
                    ${i === active ? 'text-primary font-bold bg-primary/10' : 'text-primary/40 hover:text-primary/70 font-medium'}`}>
                  {hospital.name}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={goPrev} aria-label="Previous"
                className="w-9 h-9 rounded-full flex items-center justify-center
                           border border-white/20 text-white/60 hover:text-white hover:border-white/50 transition-all">
